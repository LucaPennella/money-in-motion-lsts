...
df = pd.read_csv(f"{PROCESSED}/pre-process/wstETH-transfers.csv")
# load velocities
...
# delete addresses with negative balance
# delete addresses with negative balance
neg_addr = [addr for addr in balances.keys() if (balances[addr] < -0.1).any()]
for addr in neg_addr:
    del balances[addr]
...
# save
rec.to_parquet(f'{OUTPUT}/wstETH-categories.parquet')
